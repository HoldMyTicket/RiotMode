...
riot.tag('rm-modal', '<div class="modalMaster" show="{open}"> <div show="{open}" class="overlay" onclick="{closeModalIfNotObtrusive}"></div> <div class="modal"> <button hide="{opts.obtrusive}" class="close-btn" onclick="{closeModalIfNotObtrusive}">X</button> <div class="modal-content"><yield></yield></div> <div class="clear"></div> </div> </div> <button hide="{hide_btn}" onclick="{ openModal }" class="{ opts[\'open-btn-class\'] }"><i class="{ opts[\'open-btn-icon\'] }"></i> { opts[\'open-btn-text\'] }</button>', 'rm-modal .modalMaster { position: fixed; top: 0; right: 0; bottom: 0; left: 0; z-index: 100; overflow: auto; } rm-modal .overlay { position: absolute; top: 0; right: 0; bottom: 0; left: 0; text-align: center; z-index: 100; background-color: rgba(0, 0, 0, 0.8); } rm-modal .modal { max-width: 35%; position: absolute; left: 50%; padding: 15px; background-color: #fff; z-index: 102; margin-bottom: 50px; margin: 50px 0; -webkit-border-radius: 5px; -moz-border-radius: 5px; -o-border-radius: 5px; border-radius: 5px; } rm-modal .close-btn { background-color: black; color: white; font-weight: 200; width: 30px; height: 30px; border: 2px solid white; outline: none; cursor: pointer; font-size: 15px; position: absolute; top: -10px; right: -10px; -webkit-border-radius: 15px; -moz-border-radius: 15px; -o-border-radius: 15px; border-radius: 15px; -webkit-box-shadow: 0 0 5px 0 rgba(0,0,0,0.75); -moz-box-shadow: 0 0 5px 0 rgba(0,0,0,0.75); box-shadow: 0 0 5px 0 rgba(0,0,0,0.75); } rm-modal .close-btn:hover { background-color: white; color: black; -webkit-transition: all 0.3s ease-in-out; -moz-transition: all 0.3s ease-in-out; -o-transition: all 0.3s ease-in-out; transition: all 0.3s ease-in-out; } rm-modal .clear { clear: both; } rm-modal .hidden { display: none; } @media only screen and (max-width: 767px) { rm-modal .modal { max-width: 100%; width: 100% !important; margin-left: 0 !important; left: 0; } rm-modal .close-btn { right: 0; } }', function(opts) {
...
riot.tag('rm-select', '<div class="wrap noselect{opts.noborder ? \' noborder\' : \'\'}"> <input type="text" class="mdl-textfield__input base { border : select }" autocomplete="off" placeholder="{ opts.placeholder || \'Type...\' }" value="{ value }" onkeydown="{noFilter ? closeKey : \'\'}"> <input type="text" name="{opts.name}" value="{ data_value }" hidden> <div show="{ open && select && !noFilter }" class="filter"> <input type="text" class="filter-input" placeholder="{ opts.filter_placeholder || \'Filter...\' }" onkeydown="{ handleText }" autocomplete="off"> </div> <div show="{ open }" class="list-container"> <ul class="list"> <li class="list-row" show="{ noResults && value.length > 1}"> { noResultsMessage } </li> <li class="list-row item{ item.active ? \' active\' : \'\'}" onclick="{ parent.pick }" each="{ item, i in filteredList }" onclick="{ parent.select }" data-value="{ item.value || item.text }"> {item.text} </li> </ul> </div> </div>', 'rm-select * { box-sizing: border-box; } rm-select .active { background:rgb(215,215,215); } rm-select .base { height:100%; padding-left:5px; margin-bottom:0px; width:100%; } rm-select .noborder .border { border: 0; } rm-select .border { height:35px; padding-left:5px; border:1px solid rgba(0,0,0,.12); box-sizing:border-box; } rm-select .border:-moz-placeholder { color: rgb(169,169,169); } rm-select .border:-ms-input-placeholder { color: rgb(169,169,169); } rm-select .border::-webkit-input-placeholder { color: rgb(169,169,169); } rm-select .border::-moz-placeholder { color: rgb(169,169,169); } rm-select .err {border: 1px dashed red; color: rgb(169,169,169);} rm-select .filter {position:absolute;padding:0;margin:0px;width:100%;z-index:4;background:#fff;} rm-select .filter:hover {background:none;} rm-select .filter-input { background:none; border:none; border:1px solid rgba(0, 0, 0, 0.117647); border-top:none; box-sizing:border-box; color: rgb(85, 85, 85); padding:5px; font-size:16px; height:35px; margin:0px; width:inherit; } rm-select .list-container { position:absolute; width:100%; top:70px; background:#fff; height:auto; overflow-x:hidden; overflow-y:auto; border: 1px solid rgba(0, 0, 0, 0.117647); border-top:none; box-shadow: rgb(68, 68, 68) 0px 2px 10px -4px; z-index: 3; } rm-select .noselect { -webkit-touch-callout: none; -webkit-user-select: none; -khtml-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; } rm-select .list { list-style-type: none; padding:0; margin:0; -webkit-margin-before: 0; -webkit-margin-after: 0; } rm-select .list .list-row { display: block; padding:5px 15px; margin:0px; overflow:auto; border-bottom: 1px solid rgba(0, 0, 0, 0.117647); } rm-select .list .list-row .accent { position:absolute; top:0; right:0; padding:5px; font-size:12px; font-style:italic; color:rgb(169,169,169); -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; } rm-select .list .list-row:last-child { border-bottom:none; } rm-select .list .list-row:hover { background: rgb(240, 240, 240); cursor: pointer; } rm-select textarea:focus,rm-select input:focus { outline: 0; } rm-select .wrap { position: relative; height:35px; }', function(opts) {
...
riot.tag('rm-toast', '<div show="{displayToast}" class="message-container"> <div onclick="{hideToast}" class="toast { toastPosition }">{ parseHTML(opts.text) }</div> </div>', 'rm-toast .toast { position: absolute; margin: 20px; max-width: 200px; color: rgba(255, 255, 255, 1); background-color: rgba(0, 0, 0, 0.8); padding: 20px; z-index: 10; -webkit-border-radius: 5px; -moz-border-radius: 5px; -o-border-radius: 5px; border-radius: 5px; } rm-toast .top-left { top: 0; left: 0; } rm-toast .top-right { top: 0; right: 0; } rm-toast .bottom-left { left: 0; bottom: 0; } rm-toast .bottom-right { right: 0; bottom: 0; }', function(opts) {
